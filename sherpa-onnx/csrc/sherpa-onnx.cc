...
for a list of pre-trained models to download.
  ...
  for (;;) {
    ...
    std::cout<<"QQ7 begin recognizer.DecodeStreams "<<std::endl;
    recognizer.DecodeStreams(ready_streams.data(), ready_streams.size());
    //break; //qjqj: here rm
    std::cout<<"QQ8 end recognizer.DecodeStreams "<<std::endl;
  }
  ...
  for (int32_t i = 1; i <= po.NumArgs(); ++i) {
    ...
    os << r.text << "\n";
    os << r.AsJsonString() << "\n\n";
    std::cout<<"QQ9 end"<<std::endl;
  }

  std::cerr << os.str();
    std::cout<<"QQ10 end"<<std::endl;
